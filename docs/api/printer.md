...
| `area` | int | 否，默认 1 | 区域，仅 `vendor=1`（飞鹅云打印）生效：1 国内（`api.feieyun.cn`），2 东南亚（`api.jp.feieyun.com`） |
...
4. 打印内容包含：门店名称、出单序号（每日从1递增，居中加粗突出显示，用于叫号）、订单号、桌号、下单时间、商品明细（名称 x 数量）、合计金额、订单备注；用户可控文本（桌号、商品名、备注）会剥离 `<` `>` 字符，避免破坏厂商小票标签指令。
