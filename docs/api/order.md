...
| `items` | array | 是 | 同结算预览 |
| `table_id` | int | 是 | 桌号ID |
| `pay_type` | int | 是 | 1 微信支付，2 余额(本金)支付，3 酒水卡(赠金)支付，4 饮品卡支付。2/3/4 均为**独立扣款**：本单必须能被该账户全额覆盖才允许使用，不会跟其他账户组合抵扣 |
...
2. 非微信支付：会员行加锁，仅扣选中的单一账户（本金/赠金/饮品卡），不跨账户组合。
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `pay_type` | int | 是 | 同创建订单：1 微信支付，2 余额(本金)支付，3 酒水卡支付，4 饮品卡支付 |
